...
combat_instructions = O+"The combat in this game operates with random number commands, not dissimilar to the dice rolls of Dungeons and Dragons. From here on, we will refer to the calling of these commands as 'dice rolls'.\nEach enemy has a Health Points value, as does the player. Each time a dice roll occurs, the result of that roll will be subtracted from the enemy, or from the player for an enemy roll. \nThe number from the dice roll can also be affected by multiple things. Both player and enemy attacks can be buffed by items or effects. They can also by reduced by other items or effects.\n" +W
...
# Rolling a D10 (with weapon)

def armed_dice_roll():
    return random.randint(1, 10)

# Rolling a D10 (without weapon)

def unarmed_dice_roll():
    return random.randint(1, 3)

# Player Turn (no buff)

def player_turn():
    global Monster1_hp, Player_hp
    while Player_hp > 0 and Monster1_hp > 0:
        res = input(P+f"Would you like to use your {player.player_weapon} or attempt an unarmed strike? \n" + G+f"[1] Use your {player.player_weapon} \n[2] Attack with your bare hands\n"+W)
        if res == "1":
            player_roll = armed_dice_roll()
            Monster1_hp -= player_roll
            print(f"You rolled a {player_roll}. The monster's HP is now {Monster1_hp}")
        ...
        if Player_hp <= 0:
            print("The monster has defeated you. Better luck next time.")
            exit()
        
# Player Turn (with buff)

def player_turn_buff():
    global Monster1_hp, Player_hp
    while Player_hp > 0 and Monster1_hp > 0:
        res = input(P+f"Would you like to use your {player.player_weapon} or attempt an unarmed strike? \n" + G+f"[1] Use your {player.player_weapon} \n[2] Attack with your bare hands\n"+W)

        if res == "1":
            player_roll = armed_dice_roll() + 5
            Monster1_hp -= player_roll
...
def monster_turn():
    global Monster1_hp, Player_hp
    while Player_hp > 0 and Monster1_hp > 0:
        monster_roll = armed_dice_roll()
        Player_hp -= monster_roll

        print(f"Monster rolled {monster_roll}. Monster HP: {Monster1_hp}, Player HP: {Player_hp}")
...
# Monster Turn (with player buff)

def monster_turn_buff():
    global Player_hp, Monster1_hp
    while Player_hp > 0 and Monster1_hp > 0:
        monster_roll = armed_dice_roll()
        Player_hp -= monster_roll
...
def room4():
    print(P+"About halfway up the tower, you notice a doorway, half hidden by ivy, that leads off the stairs.")
...
def room4r():
    print(P+"You push the ivy aside and squeeze through the heavy, wooden door. On the other side is a small room with a solitary window.")
    print("You catch glimpses of the tops of trees through the window.In the middle of the room is an old looking chest.")
    res = input("Would you like to try and open the chest or continue on up the stairs?\n" + G+ "[1] Open the chest \n[2] Continue up the stairs\n"+W)
    if res == "1":
        print(P+f"You open the chest to see a faint glow which quickly dissipates. Just as it does, your {player.player_weapon} begins to glow and emmenates a warmth that it did not before."+W)
        print("You leave the room and continue on up the stairs")
        return room5b()
    elif res == "2":
        return room5()
    else:
...
# Monster (no item buff)

def room5():
    print("There is a monster")
    res = input(P+"Would you like to read the combat instructions?\n" + G+ "[1] Yes \n[2] No\n"+W)
    if res == "1":
        print(combat_instructions)
        return player_turn()
    elif res == "2":
        return player_turn()
    else:
